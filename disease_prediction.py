...
'Impetigo':40}},inplace=True) #inplace=True returns None 
...
def NaiveBayes():
    ...
    print("Accuracy of the test is :",accuracy_score(y_test, y_pred))
    print("Number of sample predicted to be true :",accuracy_score(y_test, y_pred, normalize=False))
...
OPTIONS = sorted(l1)
S1En=ttk.Combobox(root,textvariable=Symptom1,value=OPTIONS)
S1En.grid(row=7, column=2)
S2En=ttk.Combobox(root,textvariable=Symptom2,value=OPTIONS)
S2En.grid(row=8, column=2)
S3En=ttk.Combobox(root,textvariable=Symptom3,value=OPTIONS)
S3En.grid(row=9, column=2)
S4En=ttk.Combobox(root,textvariable=Symptom4,value=OPTIONS)
S4En.grid(row=10, column=2)
S5En=ttk.Combobox(root,textvariable=Symptom5,value=OPTIONS)
...
NameLb.grid(row=15, column=1, pady=10,  sticky=W)
